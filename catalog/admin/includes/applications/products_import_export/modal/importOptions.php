...
/**
  ...
  @version    $Id: importOptions.php v1.0 2013-12-03 resultsonlyweb $
*/
?>
<script>
...
function importOptionGroups(owizard, otype, obackup, data) {
  if ($('#optionsGroupsFile').val()) {
    ...
    $.ajaxFileUpload({
      ...
      dataType: 'json',
      data: {
        products_import_export: 'test', 
        action:'importOptionGroups'
      },
...
function reportImportOptions(owizard, otype, obackup, data) {
  var mcontent = '';
  if (data.OGdata) {
    mcontent += '<p id="importOptionsConfirmTitle"><?php echo $lC_Language->get('option_groups'); ?>:</p>'+
                '<p id="importOptionsConfirmMessage">' + data.OGdata.total + ' <?php echo $lC_Language->get('text_records_imported'); ?></p>'+
                '<p id="importOptionsConfirmMessage">' + data.OGdata.matched + ' <?php echo $lC_Language->get('text_matched_and_updated'); ?></p>'+
                '<p id="importOptionsConfirmMessage">' + data.OGdata.inserted + ' <?php echo $lC_Language->get('text_new_records_added'); ?></p>';
  }
  if (data.OVdata) {
    mcontent += '<p id="importOptionsConfirmTitle"><?php echo $lC_Language->get('option_variants'); ?>:</p>'+
                '<p id="importOptionsConfirmMessage">' + data.OVdata.total + ' <?php echo $lC_Language->get('text_records_imported'); ?></p>'+
                '<p id="importOptionsConfirmMessage">' + data.OVdata.matched + ' <?php echo $lC_Language->get('text_matched_and_updated'); ?></p>'+
                '<p id="importOptionsConfirmMessage">' + data.OVdata.inserted + ' <?php echo $lC_Language->get('text_new_records_added'); ?></p>';
  }
  if (data.OPdata) {
    mcontent += '<p id="importOptionsConfirmTitle"><?php echo $lC_Language->get('options_to_products'); ?>:</p>'+
                '<p id="importOptionsConfirmMessage">' + data.OPdata.total + ' <?php echo $lC_Language->get('text_records_imported'); ?></p>'+
                '<p id="importOptionsConfirmMessage">' + data.OPdata.matched + ' <?php echo $lC_Language->get('text_matched_and_updated'); ?></p>'+
                '<p id="importOptionsConfirmMessage">' + data.OPdata.inserted + ' <?php echo $lC_Language->get('text_new_records_added'); ?></p>';
  }
  $.modal({
    content: mcontent,
