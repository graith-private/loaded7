...
class Loaded7_SSO_Connector extends lC_Addon { // your addon must extend lC_Addon
  ...
  */
  public function Loaded7_SSO_Connector() {    
    ...
    */     
    $this->_thumbnail = lc_image(DIR_WS_CATALOG . 'addons/' . $this->_code . '/images/logo.png', $this->_title);
   /**
  ...
  */
  public function install() {
    ...
    $lC_Database->simpleQuery("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Remote Account Information URL', 'ADDONS_CONNECTORS_" . strtoupper($this->_code) . "_REMOTE_ACCOUNT_INFO_URL', '', 'Put Remote Account Information URL given in Wordpress Loaded7 SSO Options', '6', '0', 'lc_cfg_set_textarea_field', now())");
    $lC_Database->simpleQuery("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Remote Password URL', 'ADDONS_CONNECTORS_" . strtoupper($this->_code) . "_REMOTE_PASSWORD_URL', '', 'Put Remote Password URL given in Wordpress Loaded7 SSO Options', '6', '0', 'lc_cfg_set_textarea_field', now())");
  ...
  public function getKeys() {
    if (!isset($this->_keys)) {
      $this->_keys = array('ADDONS_CONNECTORS_' . strtoupper($this->_code) . '_STATUS',
                           ...
                           'ADDONS_CONNECTORS_' . strtoupper($this->_code) . '_REMOTE_ACCOUNT_INFO_URL',
                           'ADDONS_CONNECTORS_' . strtoupper($this->_code) . '_REMOTE_PASSWORD_URL',
